...
def _conn() -> sqlite3.Connection:
    ...
    conn.execute(
        ...
        ('t1', '2025-04-01', '2025-04', -900.00, 'InsurerCo Prem Pay', 'Insurance', 'Expense', 'unknown', 'now'),
        ('t2', '2024-04-01', '2024-04', -890.00, 'InsurerCo Prem Pay', 'Insurance', 'Expense', 'unknown', 'now'),
        ('t3', '2026-04-01', '2026-04', -900.00, 'InsurerCo Prem Pay', 'Insurance', 'Expense', 'unknown', 'now')
        """
    )
    conn.commit()
    return conn
...
class CadenceInsightsTests(unittest.TestCase):
    def test_find_merchant_from_text(self):
        conn = _conn()
        mk = find_merchant_key_from_text(conn, "InsurerCo is annual insurance")
        self.assertEqual(mk, "InsurerCo Prem Pay")
    ...
    def test_fallback_uses_hint(self):
        ...
        stats = {
            ...
            "months_active": 3,
            "top_amounts": [{"amount": -900.00, "count": 3}],
